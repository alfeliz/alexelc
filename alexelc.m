...
#			 			 trian		To make a triangle resistance.
...
if (fileinfo == -1) 
   error ("Script alexelc: Unable to open file name: %s, %s",infoshot, msg); 
endif;
...
if (exist("volt","var")==0)
  warning("Script alexelc: There is no voltage signal in shot %s",shot);
elseif (exist("dint","var")==0)
  warning("Script alexelc: There is no intensity signal in shot %s",shot);
elseif (exist("phot","var")==0)
  warning("Script alexelc: There is no photodiode signal in shot %s",shot);
endif;
...
if (exist("volt","var")==1)
  ...
  if (puntosv<=10)
    warning("Script alexelc: only %u points in voltage baseline",puntosv);
  endif;
...
if (exist("phot","var")==1)
  ...
  if (puntosp<=10)
    warning("Script alexelc: only %u points in photodiode baseline",puntosp);
  endif;
...
if (exist("dint","var")==1)
  ...
  if (puntosp<=10)
    warning("Script alexelc: only %u points in d(i)/dt baseline",puntosp);
  endif;
...
rtrian = trian(5.5, 1.3 , 0.01, abs(dint(11,1)-dint(10,1)) ); %Triangular signal for the resistance.

####[mv,idx] = max(int); %triangular resistance starts at the time where maximum current is.

graint = gradient(int); %Current gradient.

id = find(gradient(int)>10); %Put in an index vector positions of current gradient positive, to know were the current starts.

idx = id(1)-60;


disp("Time to start triangular resistance: (µs)")
dint(idx,1) %Time for r(t) to start. Beginning of current.

res(1+idx:length(rtrian)+idx) = rtrian + 0;
...
plot(valt(:,1),valt(:,2),"*r"); %To see the voltage through the wire as purely resistive.
hold;
plot(volt(:,1),volt(:,2),"+k");
...
#Derivative voltage (if there is)
if (exist("Vres","var"))
  #Output file name:
  name = horzcat(shot,"_der_volt.txt"); %Adding the right sufix to the shot name.
  output = fopen(name,"w"); %Opening the file.
  #First line:
  fdisp(output,"descriptor `time(micros)`  `der res. v(V/µs)`"); %Veusz format
  redond = [2 3]; %Saved precision 
  vol = [volt(:,1), gradient(Vres(:,2))];
  display_rounded_matrix(vol, redond, output); %This function is not made by my.
  fclose(output); %Closing the file.
  disp("Derivative res. voltage saved.");
endif;

#Derivative reisistance
  #Output file name:
  name = horzcat(shot,"_der_res.txt"); %Adding the right sufix to the shot name.
  output = fopen(name,"w"); %Opening the file.
  #First line:
  fdisp(output,"descriptor `time(micros)`  `der res(Ohm/µs)`"); %Veusz format
  redond = [2 3]; %Saved precision 
  vol = [volt(:,1), ( gradient(Vres(:,2)).*int + Vres(:,2).*dint(:,2) ) / (int.**2)];
  display_rounded_matrix(vol, redond, output); %This function is not made by my.
  fclose(output); %Closing the file.
  disp("Derivative res. saved.");
...
#Current derivative (if there is)
if (exist("dint","var"))
  #Output file name:
  name = horzcat(shot,"_deri.txt"); %Adding the right sufix to the shot name.
  output = fopen(name,"w"); %Opening the file.
  #First line:
  fdisp(output,"descriptor `time(micros)`  `der curr(A/µs)`"); %Veusz format
  redond = [2 3]; %Saved precision 
  display_rounded_matrix(dint, redond, output); 
  fclose(output); %Closign the file.
  disp("Der curr. saved.");
endif;
...
###
timing = toc;
disp("Script alexelc execution time:")
disp(timing)
disp(" seconds")
